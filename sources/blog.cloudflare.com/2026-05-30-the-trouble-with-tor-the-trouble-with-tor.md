...
fetched_at: 2026-05-30T17:21:40
content_hash: sha1:8cc9f9f83bb568bea6c03a0a1e32234cdc7c25aa
extractor: trafilatura
---

[The Tor Project](https://www.torproject.org/) makes a browser that allows anyone to surf the Internet anonymously. Tor stands for "the onion router" and that describes how the service works. Traffic is routed through a number of relays run across the Internet where each relay only knows the next hop (because each hop is enclosed in a cryptographic envelope), not the ultimate destination, until the traffic gets to the final exit node which connects to the website — like peeling the layers of an onion.
...
Anonymity online is important for a number of reasons we at CloudFlare believe in. For instance, Tor is instrumental in ensuring that individuals living in repressive regimes can access information that may otherwise be blocked or illegal. We believe this is so important that we offer our service for free through [Project Galileo](https://www.cloudflare.com/galileo/) to protect politically and artistically important organizations and journalists against attacks that would otherwise censor their work.

On the other hand, anonymity is also something that provides value to online attackers. Based on data across the CloudFlare network, 94% of requests that we see across the Tor network are per se malicious. That doesn’t mean they are visiting controversial content, but instead that they are automated requests designed to harm our customers. A large percentage of the comment spam, vulnerability scanning, ad click fraud, [content scraping](https://www.cloudflare.com/learning/ai/how-to-prevent-web-scraping/), and login scanning comes via the Tor network. To give you some sense, based on data from [Project Honey Pot](http://www.projecthoneypot.org/), 18% of global email spam, or approximately 6.5 trillion unwanted messages per year, begin with an automated bot harvesting email addresses via the Tor network.

[CC BY 3.0](https://creativecommons.org/licenses/by/3.0/us/) logo from [the Tor Project](https://www.torproject.org/docs/trademark-faq.html.en)
...
Since the list of [Tor exit nodes](https://check.torproject.org/exit-addresses) is public we’ve open sourced our tools used to examine the reputation and life of exit nodes. [torhoney](https://github.com/jgrahamc/torhoney) matches Tor exit nodes and information from Project Honey Pot to see which nodes are most misused and [torexit](https://github.com/jgrahamc/torexit) can be used to determine the longevity of Tor exit nodes. We used it to tune our download of the Tor exit node list to ensure that we were up to date.
