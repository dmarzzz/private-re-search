---
url: https://julia.quantecon.org/dynamic_programming/wald_friedman.html
title: A Problem that Stumped Milton Friedman
fetched_at: 2026-05-30T20:26:31
content_hash: sha1:a07863485e717e479ff0bfa12492168a614a1599
extractor: trafilatura
---

[31. A Problem that Stumped Milton Friedman](https://julia.quantecon.org#id9)[#](https://julia.quantecon.org#a-problem-that-stumped-milton-friedman)
...
[31.1. Overview](https://julia.quantecon.org#id10)[#](https://julia.quantecon.org#overview)
...
This problem led Abraham Wald [[Wal47](https://julia.quantecon.org/zreferences.html#id32)] to formulate **sequential analysis**, an approach to statistical decision problems intimately related to dynamic programming.
...
**sequential probability ratio test**The

**power**of a statistical testThe

**critical region**of a statistical testA

**uniformly most powerful test**

[31.2. Origin of the problem](https://julia.quantecon.org#id11)[#](https://julia.quantecon.org#origin-of-the-problem)

On pages 137-139 of his 1998 book *Two Lucky People* with Rose Friedman [[FF98](https://julia.quantecon.org/zreferences.html#id28)],
...
That started Wald on the path that led him to *Sequential Analysis* [[Wal47](https://julia.quantecon.org/zreferences.html#id32)].

We’ll formulate the problem using dynamic programming.

[31.3. A dynamic programming approach](https://julia.quantecon.org#id12)[#](https://julia.quantecon.org#a-dynamic-programming-approach)

The following presentation of the problem closely follows Dmitri
Berskekas’s treatment in **Dynamic Programming and Stochastic Control** [[Ber75](https://julia.quantecon.org/zreferences.html#id31)].
...
This is a mixture of distributions \(f_0\) and \(f_1\), with the weight on \(f_0\) being the posterior probability that \(x = x_0\) [1](https://julia.quantecon.org#f1).
...
### 31.3.1. Losses and costs[#](https://julia.quantecon.org#losses-and-costs)
...
### 31.3.2. Digression on type I and type II errors[#](https://julia.quantecon.org#digression-on-type-i-and-type-ii-errors)
...
### 31.3.3. Intuition[#](https://julia.quantecon.org#intuition)
...
### 31.3.4. A Bellman equation[#](https://julia.quantecon.org#a-bellman-equation)
...
[#](https://julia.quantecon.org#equation-new1)\[J(p) = \min \left\{ (1-p) L_0, \; p L_1, \; c + \mathbb E [ J (p') ] \right\}\]

where \(p'\) is the random variable defined by

[#](https://julia.quantecon.org#equation-wf-new2)\[p' = \frac{ p f_0(z)}{ p f_0(z) + (1-p) f_1 (z) }\]

when \(p\) is fixed and \(z\) is drawn from the current best guess, which is the distribution \(f\) defined by

[#](https://julia.quantecon.org#equation-wf-new3)\[f(v) = p f_0(v) + (1-p) f_1 (v)\]
...
[#](https://julia.quantecon.org#equation-new4)\[A(p) := \mathbb E [ J (p') ]\]

Then we can represent the Bellman equation as

[#](https://julia.quantecon.org#equation-new5)\[J(p) = \min \left\{ (1-p) L_0, \; p L_1, \; c + A(p) \right\}\]
...
[#](https://julia.quantecon.org#equation-new6)\[(1- p) L_0 < \min \{ p L_1, c + A(p) \} \textrm { if } p \geq \alpha\]

and

[#](https://julia.quantecon.org#equation-new7)\[p L_1 < \min \{ (1-p) L_0, c + A(p) \} \textrm { if } p \leq \beta\]

The optimal decision rule is then

[#](https://julia.quantecon.org#equation-new8)\[\begin{split}\begin{aligned} \textrm { accept } x=x_0 \textrm{ if } p \geq \alpha \\ \textrm { accept } x=x_1 \textrm{ if } p \leq \beta \\ \textrm { draw another } z \textrm{ if } \beta \leq p \leq \alpha \end{aligned}\end{split}\]
...
[31.4. Implementation](https://julia.quantecon.org#id13)[#](https://julia.quantecon.org#implementation)
...
[(31.4)](https://julia.quantecon.org#equation-new4).
...
```
continuation_value (generic function with 1 method)
```

Next we solve a problem by applying value iteration to compute the value function and the associated decision rule.
...
```
Correct: 0.91, Average Cost: 0.54, Average number of trials: 1.81
```

### 31.4.1. Comparative statics[#](https://julia.quantecon.org#comparative-statics)
...
```
Correct: 0.79, Average Cost: 0.86, Average number of trials: 1.09
```
...
[31.5. Comparison with Neyman-Pearson formulation](https://julia.quantecon.org#id14)[#](https://julia.quantecon.org#comparison-with-neyman-pearson-formulation)
...
We’ll rely on Abraham Wald’s [[Wal47](https://julia.quantecon.org/zreferences.html#id32)] elegant summary of Neyman-Pearson theory.

For our purposes, watch for there features of the setup:

the assumption of a

*fixed*sample size \(n\)the application of laws of large numbers, conditioned on alternative probability models, to interpret the probabilities \(\alpha\) and \(\beta\) defined in the Neyman-Pearson theory
...
In chapter 1 of **Sequential Analysis** [[Wal47](https://julia.quantecon.org/zreferences.html#id32)] Abraham Wald summarizes the
