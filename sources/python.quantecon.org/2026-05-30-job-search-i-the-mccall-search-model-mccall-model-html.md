---
url: https://python.quantecon.org/mccall_model.html
title: Job Search I: The McCall Search Model
fetched_at: 2026-05-30T19:05:49
content_hash: sha1:110971c7adaf3e7129d18339405031ea7bc5f6c2
extractor: trafilatura
---

[49. Job Search I: The McCall Search Model](https://python.quantecon.org#id2)[#](https://python.quantecon.org#job-search-i-the-mccall-search-model)
...
[49.1. Overview](https://python.quantecon.org#id3)[#](https://python.quantecon.org#overview)

The McCall search model [[McCall, 1970](https://python.quantecon.org/zreferences.html#id291)] helped transform economists’ way of thinking about labor markets.
...
[49.2. The McCall Model](https://python.quantecon.org#id4)[#](https://python.quantecon.org#the-mccall-model)
...
[#](https://python.quantecon.org#equation-obj-model)\[{\mathbb E} \sum_{t=0}^\infty \beta^t y_t\]
...
### 49.2.1. A Trade-Off[#](https://python.quantecon.org#a-trade-off)
...
### 49.2.2. The Value Function[#](https://python.quantecon.org#the-value-function)

In order to optimally trade-off current and future rewards, we need to think about two things:

the current payoffs we get from different choices

the different states that those choices will lead to in next period
...
More precisely, \(v^*(w)\) denotes the total sum of expected discounted earnings when an agent always behaves in an optimal way. points in time.

Of course \(v^*(w)\) is not trivial to calculate because we don’t yet know what decisions are optimal and what aren’t!

If we don’t know what opimal choices are, it feels imposible to calculate \(v^*(w)\).

But let’s put this aside for now and think of \(v^*\) as a function that assigns to each possible wage \(w\) the maximal lifetime value \(v^*(w)\) that can be obtained with that offer in hand.

A crucial observation is that this function \(v^*\) must satisfy

[#](https://python.quantecon.org#equation-odu-pv)\[v^*(w) = \max \left\{ \frac{w}{1 - \beta}, \, c + \beta \sum_{w' \in \mathbb{W}} v^*(w') q (w') \right\}\]
...
the first term inside the max operation is the lifetime payoff from accepting current offer, since such a worker works forever at \(w\) and values this income stream as
...
But this is precisely \(v^*(w)\), which is the left-hand side of [(49.2)](https://python.quantecon.org#equation-odu-pv).

Putting this all together, we see that [(49.2)](https://python.quantecon.org#equation-odu-pv) is valid for all \(w\).

### 49.2.3. The Optimal Policy[#](https://python.quantecon.org#the-optimal-policy)

We still don’t know how to compute \(v^*\) (although [(49.2)](https://python.quantecon.org#equation-odu-pv) gives us hints
we’ll return to below).

But suppose for now that we do know \(v^*\).

Once we have this function in hand we can easily make optimal choices (i.e., make the right choice between accept and reject given any \(w\)).

All we have to do is select the maximal choice on the right-hand side of [(49.2)](https://python.quantecon.org#equation-odu-pv).

In other words, we make the best choice between stopping and continuing, given the information provided to us by \(v^*\).

The optimal action is best thought of as a **policy**, which is, in general, a map from
states to actions.

Given any \(w\), we can read off the corresponding best choice (accept or
reject) by picking the max on the right-hand side of [(49.2)](https://python.quantecon.org#equation-odu-pv).
...
[#](https://python.quantecon.org#equation-reswage)\[ \bar w := (1 - \beta) \left\{ c + \beta \sum_{w'} v^*(w') q (w') \right\}\]
...
In view of [(49.3)](https://python.quantecon.org#equation-reswage), we can compute this reservation wage if we can compute the value function.

[49.3. Computing the Optimal Policy: Take 1](https://python.quantecon.org#id5)[#](https://python.quantecon.org#computing-the-optimal-policy-take-1)
...
In view of [(49.2)](https://python.quantecon.org#equation-odu-pv), this vector satisfies the nonlinear system of equations

[#](https://python.quantecon.org#equation-odu-pv2)\[v^*(i) = \max \left\{ \frac{w(i)}{1 - \beta}, \, c + \beta \sum_{j=1}^n v^*(j) q (j) \right\} \quad \text{for } i = 1, \ldots, n\]

### 49.3.1. The Algorithm[#](https://python.quantecon.org#the-algorithm)
...
[#](https://python.quantecon.org#equation-odu-pv2p)\[v'(i) = \max \left\{ \frac{w(i)}{1 - \beta}, \, c + \beta \sum_{j=1}^n v(j) q (j) \right\} \quad \text{for } i = 1, \ldots, n\]
...
### 49.3.2. Fixed Point Theory[#](https://python.quantecon.org#fixed-point-theory)

What’s the mathematics behind these ideas?

First, one defines a mapping \(T\) from \(\mathbb R^n\) to itself via

[#](https://python.quantecon.org#equation-odu-pv3)\[(Tv)(i) = \max \left\{ \frac{w(i)}{1 - \beta}, \, c + \beta \sum_{j=1}^n v(j) q (j) \right\} \quad \text{for } i = 1, \ldots, n\]
...
### 49.3.3. Implementation[#](https://python.quantecon.org#implementation)
...
for our model class to maintain immutability, which works well with JAX’s functional programming paradigm.
...
We implement the Bellman operator \(T\) from [(49.6)](https://python.quantecon.org#equation-odu-pv3), which we can write in
terms of array operations as

[#](https://python.quantecon.org#equation-odu-pv4)\[Tv = \max \left\{ \frac{w}{1 - \beta}, \, c + \beta \sum_{j=1}^n v(j) q (j) \right\} \quad\]

(The first term inside the max is an array and the second is just a number – here we mean that the max comparison against this number is done element-by-element for all elements in the array.)
...
### 49.3.4. Comparative Statics[#](https://python.quantecon.org#comparative-statics)
...
The reservation wages will be plotted alongside the wage offer distribution, so that we can get a sense of what fraction of offers will be accepted.
...
As a first step, given that we’ll use it many times, let’s create a more efficient, jit-complied version of the function that computes the reservation wage:
...
[49.4. Computing an Optimal Policy: Take 2](https://python.quantecon.org#id6)[#](https://python.quantecon.org#computing-an-optimal-policy-take-2)
...
[#](https://python.quantecon.org#equation-j1)\[ h = c + \beta \sum_{w'} v^*(w') q (w')\]

The Bellman equation can now be written as

[#](https://python.quantecon.org#equation-j1b)\[ v^*(w') = \max \left\{ \frac{w'}{1 - \beta}, \, h \right\}\]

Now let’s derive a nonlinear equation for \(h\) alone.

Starting from [(49.9)](https://python.quantecon.org#equation-j1b), we multiply both sides by \(q(w')\) to get
...
Finally, using the definition of \(h\) from [(49.8)](https://python.quantecon.org#equation-j1), the left-hand side is just \(h\), giving us

[#](https://python.quantecon.org#equation-j2)\[ h = c + \beta \sum_{w' \in \mathbb W} \max \left\{ \frac{w'}{1 - \beta}, h \right\} q (w')\]
...
[#](https://python.quantecon.org#equation-j3)\[h' = c + \beta \sum_{w' \in \mathbb W} \max \left\{ \frac{w'}{1 - \beta}, h \right\} q (w') \quad\]
...
[49.5. Continuous Offer Distribution](https://python.quantecon.org#id7)[#](https://python.quantecon.org#continuous-offer-distribution)

The discrete wage offer distribution used above is convenient for theory and computation, but many realistic distributions are continuous (i.e., have a density).

Fortunately, the theory changes little in our simple model when we shift to a continuous offer distribution.

Recall that \(h\) in [(49.8)](https://python.quantecon.org#equation-j1) denotes the value of not accepting a job in this period but
then behaving optimally in all subsequent periods.

To shift to a continuous offer distribution, we can replace [(49.8)](https://python.quantecon.org#equation-j1) by

[#](https://python.quantecon.org#equation-j1c)\[h = c + \beta \int v^*(s') q (s') ds'. \quad\]

Equation [(49.10)](https://python.quantecon.org#equation-j2) becomes

[#](https://python.quantecon.org#equation-j2c)\[h = c + \beta \int \max \left\{ \frac{w(s')}{1 - \beta}, h \right\} q (s') d s' \quad\]

The aim is to solve this nonlinear equation by iteration, and from it obtain the reservation wage.

### 49.5.1. Implementation with Lognormal Wages[#](https://python.quantecon.org#implementation-with-lognormal-wages)
...
Now let’s investigate how the reservation wage changes with \(c\) and \(\beta\) using a contour plot.
...
[49.6. Volatility](https://python.quantecon.org#id8)[#](https://python.quantecon.org#volatility)

An interesting feature of the McCall model is that increased volatility in wage offers tends to increase the reservation wage.

The intuition is that volatility is attractive to the worker because they can enjoy the upside (high wage offers) while rejecting the downside (low wage offers).

Hence, with more volatility, workers are more willing to continue searching rather than accept a given offer, which means the reservation wage rises.

To illustrate this phenomenon, we use a mean-preserving spread of the wage distribution.

In particular, we vary the scale parameter \(\sigma\) in the lognormal wage distribution \(w(s) = \exp(\mu + \sigma s)\) while adjusting \(\mu\) to keep the mean constant.

Recall that for a lognormal distribution with parameters \(\mu\) and \(\sigma\), the mean is \(\exp(\mu + \sigma^2/2)\).
...
### 49.6.1. Lifetime Value and Volatility[#](https://python.quantecon.org#lifetime-value-and-volatility)
...
Higher volatility provides more upside potential, while at the same time workers can protect themselves against downside risk by rejecting low offers.

This option value translates into higher expected lifetime utility.

To demonstrate this, we will:

Compute the reservation wage for each volatility level

Calculate the expected discounted value of the lifetime income stream associated with that reservation wage, using Monte Carlo.


The simulation works as follows:

Compute the present discounted value of one lifetime earnings path, from a given wage path.

Average over a large number of such calculations to approximate expected discounted value.
...
The plot confirms that despite workers setting higher reservation wages when facing more volatile wage offers (as shown above), they achieve higher expected lifetime values due to the option value of search.

[49.7. Exercises](https://python.quantecon.org#id9)[#](https://python.quantecon.org#exercises)
